  ...
  return (
    <div className="fixed inset-0 z-[70] pointer-events-none flex flex-col items-center justify-start font-mono">
      ...
      {/* Headline banner. Constrained to a max-width and kept clear of the
          top-left corner (where the resolution math card docks), and pushed
          down past the stage tracker bar (ends ~125px from viewport top) and
          the INJECT button beneath it (ends ~177px), so it never runs into
          other on-screen overlays at narrow viewports. */}
      <div
        className={`relative mt-52 mx-4 max-w-2xl animate-outcome-pop rounded-2xl border-2 px-6 py-4 sm:px-10 sm:py-6 backdrop-blur-md text-center shadow-2xl ${
          approved
